...
function dashKmVolChart(containerId, clientes) {
  const el = document.getElementById(containerId);
  if (!el) return;
  // Inclui todos os clientes, ordenados por volume; km é opcional
  const itens = clientes.slice(0, 30);
  if (!itens.length) { el.innerHTML = '<div style="color:#000;font-size:12px;padding:12px">Sem dados</div>'; return; }
